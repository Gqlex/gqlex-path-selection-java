...
/**
 * Operation to remove an argument from a GraphQL field at a specified path.
 */
public class RemoveArgumentOperation implements TransformationOperation {
    
    private final String path;
    private final String argumentName;
    
    public RemoveArgumentOperation(String path, String argumentName) {
        this.path = path;
    ...
    public Document apply(Document document, TransformationContext context) {
        try {
            ...
            if (targetNode instanceof Field) {
                AstManipulationUtils.removeArgumentFromField((Field) targetNode, argumentName);
            } else {
                context.addError("Target node is not a field: " + targetNode.getClass().getSimpleName());
            }
            return document;
        } catch (Exception e) {
            context.addError("Error removing argument: " + e.getMessage());
            return document;
        }
    // Note: The actual AST manipulation logic is now handled by AstManipulationUtils
    // This provides a cleaner separation of concerns and reusability
    ...
    public String getDescription() {
        return String.format("Remove argument '%s' at path '%s'", argumentName, path);
    // Note: The actual AST manipulation logic is now handled by AstManipulationUtils
    // This provides a cleaner separation of concerns and reusability
}
    ...
    public OperationType getOperationType() {
        return OperationType.REMOVE_ARGUMENT;
    // Note: The actual AST manipulation logic is now handled by AstManipulationUtils
    // This provides a cleaner separation of concerns and reusability
}
    ...
    private void removeArgumentFromField(Field field, String argumentName) {
