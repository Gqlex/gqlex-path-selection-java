...
/**
 * Operation to sort fields at a specified path for consistent caching.
 */
public class SortFieldsOperation implements TransformationOperation {
    
    private final String path;
    
    public SortFieldsOperation(String path) {
        this.path = path;
    ...
    public Document apply(Document document, TransformationContext context) {
        try {
            ...
            if (targetNode instanceof SelectionSetContainer) {
                sortFields((SelectionSetContainer) targetNode);
            } else {
                context.addError("Target node does not support field sorting: " + targetNode.getClass().getSimpleName());
            }
            return document;
        } catch (Exception e) {
            context.addError("Error sorting fields: " + e.getMessage());
            return document;
        }
    // Note: The actual AST manipulation logic is now handled by AstManipulationUtils
    // This provides a cleaner separation of concerns and reusability
    ...
    public String getDescription() {
        return String.format("Sort fields at path '%s'", path);
    // Note: The actual AST manipulation logic is now handled by AstManipulationUtils
    // This provides a cleaner separation of concerns and reusability
}
    ...
    public OperationType getOperationType() {
        return OperationType.SORT_FIELDS;
    // Note: The actual AST manipulation logic is now handled by AstManipulationUtils
    // This provides a cleaner separation of concerns and reusability
}
    ...
    private void sortFields(SelectionSetContainer container) {
