...
import com.intuit.gqlex.traversal.TuneableTraversal;
...
public class SearchPathBuilder {
    ...
    private TuneableTraversal tuneableTraversal = null;
    ...
    public SearchPathBuilder(String searchSyntax) {
        ...
        if( searchSyntax.isEmpty()){
            throw new GqlSelectionSyntaxException("gqlXPath must start with // ");
        }
        ...
        if(     ! searchSyntax.startsWith(SearchElementConstants.DOUBLE_SLASH) &&
            ...
            throw new GqlSelectionSyntaxException("gqlXPath must start with // or /, gqlXPath Range can be set  before // or /.");
        }
        ...
        if( searchSyntax.startsWith("{")) {
            ...
            if (indEndRange == -1) {
                throw new GqlSelectionSyntaxException("gqlXPath Range pattern should be {x:y}");
            }
            ...
            try {
                ...
                throw new GqlSelectionSyntaxException("gqlXPath Range pattern should be {x:y}, x and y must be integer");
            }
            if (rangeStart < 0) {
                throw new GqlSelectionSyntaxException("gqlXPath Range pattern should be {rangeStart:y}, startRange must be equal or greater to zero");
            }
            ...
            if (Strings.isNullOrEmpty(endRange)) {
                ...
                if (rangeEnd < 0) {
                    throw new GqlSelectionSyntaxException("gqlXPath Range pattern should be {x:rangeEnd}, rangeEnd must be equal or greater to zero");
                }
                selectionRange.setRangeEnd(rangeEnd);
            }

            if( selectionRange.getRangeStart() > selectionRange.getRangeEnd()){
                throw new GqlSelectionSyntaxException("gqlXPath Range start value must be less or equal to end value {x:y}");
            }
        ...
        if( searchSyntax.startsWith(SearchElementConstants.DOUBLE_SLASH)) {
            ...
            if (selectionRange != null) {
                throw new GqlSelectionSyntaxException("gqlXPath Range cannot be set with /.");
            }
        ...
        tuneableTraversal = new TuneableTraversal(pathElementsToSearch);
           ...
           // List<String> reduceArray = new ArrayList<>();
            for (String element : pathElementsToSearch) {
                ...
                if( Strings.isNullOrEmpty(pathElementValue) ){
                    throw new GqlSelectionSyntaxException("gqlXPath contains empty names.");
                ...
                if (i <= length - 1) {
                    ...
                    if( containsAny) {
                        if (i < length - 1) {
                            ...
                            throw new GqlSelectionSyntaxException("Last gqlXPath element cannot contains of ... [any]");
                        }
                    }
                    ...
                    if (!matcher.find()) {
                        throw new GqlSelectionSyntaxException("gqlXPath can contains only string (numbers, letters and -, =,_ , ... )");
                    }
        ...
        if( ! isContainsFieldName){
            throw new GqlSelectionSyntaxException("gqlXPath contains only any [...], please set valid path with targeted node name.");
        }

        buildStrategy();
    }

    public TuneableTraversal getTuneableTraversal() {
        return tuneableTraversal;
    }
